...
void main() {
  ...
  test('Test basic client network update', () {
    ...
    TestConnection connectionB = new TestConnection('b');
    TestConnection connectionC = new TestConnection('c');
    ...
    when(mockSpriteIndex[1000]).thenReturn(new _TestSprite.withVecPosition(1000, new Vec2(9, 9)));

    network.frame(0.01, new List());

    // Full state sent over network.
    expect(connectionC.decodedRecentDataRecevied()['1000'],
        equals([SpriteConstructor.DAMAGE_PROJECTILE.index,99,9,9,0,180000,180000,1,null,2,2,1,0]));
  });
}
